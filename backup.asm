...
main:
    ...
    # === ENABLE INTERRUPTS HERE (AFTER screen is shown) ===
    li $t0, 0xffff0000
    lw $t1, 0($t0)
    ori $t1, $t1, 2
    sw $t1, 0($t0)
...
# -------------------------
# Keyboard interrupt (Moved to bottom and made safe for $at)
.ktext 0x80000180
kb_interrupt:
    .set noat           # Tell assembler not to use $at implicitly
    move $k0, $at       # Save main program's $at state into $k0 safely
    ...
    .set noat
    move $at, $k0       # Safely restore main program's $at state
    .set at
    eret